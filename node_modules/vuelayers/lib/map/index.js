...
import Vue from 'vue';
import { featuresContainer, interactionsContainer, layersContainer, olCmp, overlaysContainer, projTransforms } from '../mixin';
import { getMapId, initializeInteraction, setMapDataProjection, setMapId, EPSG_3857, MAX_ZOOM, MIN_ZOOM, ZOOM_FACTOR } from '../ol-ext';
import { observableFromOlEvent, observableFromOlChangeEvent } from '../rx-ext';
import { hasMap, hasView } from '../util/assert';
import { isEqual, isPlainObject, arrayLengthValidator, coalesce, isFunction, noop, pick } from '../util/minilo';
...
var script = {
  ...
  props: {
    ...
    renderer: {
      ...
      validator: function validator(val) {
        ...
        return _includesInstanceProperty(_context = ['canvas', 'webgl']).call(_context, val);
      }
    }
  },
  methods: {
    ...
    mount: function mount() {
      ...
      this.$nextTick(_bindInstanceProperty(_context2 = this.updateSize).call(_context2, this));
      this.subscribeAll();
    },
  ...
  watch: _objectSpread(_objectSpread({}, makeWatchers(['keyboardEventTarget', 'loadTilesWhileAnimating', 'loadTilesWhileInteracting', 'moveTolerance', 'pixelRatio', 'renderer', 'maxTilesLoading'], function () {
    return function () {
  ...
  created: function created() {
    ...
    if (this.defaultControls instanceof Collection) {
      ...
      this._controlsCollection = defaults(isPlainObject(this.defaultControls) ? this.defaultControls : undefined);
    ...
    if (this.defaultInteractions instanceof Collection) {
      ...
      this._interactionsCollection = defaults$1(isPlainObject(this.defaultInteractions) ? this.defaultInteractions : undefined);
    }

    _forEachInstanceProperty(_context3 = this._interactionsCollection).call(_context3, function (interaction) {
  ...
  methods: {
    ...
    animate: function animate() {
      ...
      if (isFunction(args[args.length - 1])) {
        ...
        args = _sliceInstanceProperty(args).call(args, 0, args.length - 1);
      }

      _forEachInstanceProperty(args).call(args, function (opts) {
      ...
      return new _Promise(function (resolve) {
        ...
        return (_this$$view = _this.$view).animate.apply(_this$$view, _concatInstanceProperty(_context = _toConsumableArray(args)).call(_context, [function (complete) {
          cb(complete);
          resolve(complete);
        }]));
      });
    ...
    fit: function fit(geometryOrExtent) {
      ...
      return new _Promise(function (resolve) {
        _this2.$view.fit(geometryOrExtent, _objectSpread$1(_objectSpread$1({}, options), {}, {
