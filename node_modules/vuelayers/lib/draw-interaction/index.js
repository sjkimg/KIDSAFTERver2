...
import { mapTo } from 'rxjs/_esm5/internal/operators/mapTo';
import { mergeMap } from 'rxjs/_esm5/internal/operators/mergeMap';
...
function subscribeToInteractionChanges() {
  ...
  var drawEvents = merge(observableFromOlEvent(this.$interaction, 'drawstart').pipe(map(function (evt) {
    ...
    return evt;
  })), observableFromOlEvent(this.$interaction, 'drawend').pipe( // FIXME blood patch, wait for vl-vector-source featuresDataProj watcher to trigger
  mergeMap(function (evt) {
    return from(_Promise.resolve(_this2.getInstance(_this2.source))).pipe(mergeMap(function (source) {
      return from(new _Promise(function (resolve) {
        _setTimeout(resolve, 1000 / 60);
      }));
    }), mapTo(evt));
  })));
